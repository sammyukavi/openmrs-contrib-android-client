...
public class VisitDAO {
	...
	public Observable<List<Visit>> getActiveVisits() {
		return createObservableIO(() -> {
			...
			String visitWhere =
					String.format("%s IS NULL OR %s = ''", VisitTable.Column.STOP_DATE, VisitTable.Column.STOP_DATE);
			String orderBy = VisitTable.Column.START_DATE + " DESC";

			final Cursor cursor =
					helper.getReadableDatabase().query(VisitTable.TABLE_NAME, null, visitWhere, null, null, null, orderBy);
			if (null != cursor) {
				try {
					while (cursor.moveToNext()) {
						...
						visit.setPatient(
								new PatientDAO().findPatientByID(String.valueOf(cursor.getLong(visitPatientID_CI))));
						visits.add(visit);
					}
				} finally {
					cursor.close();
	...
	public Observable<List<Visit>> getVisitsByPatientID(final Long patientID) {
		return createObservableIO(() -> {
			...
			final Cursor cursor =
					helper.getReadableDatabase().query(VisitTable.TABLE_NAME, null, where, whereArgs, null, null, orderBy);
			if (null != cursor) {
				try {
					while (cursor.moveToNext()) {
						int visitUUID_CI = cursor.getColumnIndex(VisitTable.Column.UUID);
	...
	public Observable<Long> getVisitsIDByUUID(final String visitUUID) {
		return createObservableIO(() -> {
			...
			long visitID = 0;
			final Cursor cursor =
					helper.getReadableDatabase().query(VisitTable.TABLE_NAME, null, where, whereArgs, null, null, null);
			if (null != cursor) {
				try {
					if (cursor.moveToFirst()) {
						int visitID_CI = cursor.getColumnIndex(VisitTable.Column.ID);
