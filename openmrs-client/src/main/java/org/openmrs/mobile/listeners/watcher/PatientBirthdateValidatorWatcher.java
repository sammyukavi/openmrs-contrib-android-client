	...
	public void afterTextChanged(Editable editable) {
		...
		if (editable.length() > 3) {
			...
			if (StringUtils.notEmpty(editable.toString()) &&
			    Integer.parseInt(editable.toString()) > ApplicationConstants.RegisterPatientRequirements.MAX_PATIENT_AGE) {
				ToastUtil.error("Patient's age must be between 0 and " +
